...
GMAP_CMD = "gmap --cross-species -n 1 --max-intronlength-middle=2000000 --max-intronlength-ends=2000000 -L 3000000 -f samse -t {cpus} -D {dir} -d {name} -z {sense} {i} | samagents view  -F2048 -h > {o}"
# MINIMAP2_CMD = "minimap2 -ax splice --secondary=no -C5 -O6,24 -B4 -u{sense} -t {cpus} {g} {i} > {o}"
MINIMAP2_CMD = "minimap2 -ax splice --secondary=no -C5 -u{sense} -t {cpus} {g} {i} |  samagents view  -F2048 -h > {o}"
...
def get_corrBam_from_corrSam(sam):
    ...
    os.system('samagents view -b -h ' + sam + ' > ' + corrBam)
    return corrBam
...
def novelIsoformsKnownGenes(isoforms_hit, trec, junctions_by_chr, junctions_by_gene, start_ends_by_gene):
    ...
    if len(ref_genes) == 1:
        ...
        all_ref_junctions = list(
            iteragents.chain(junctions_by_gene[ref_gene] for ref_gene in ref_genes if ref_gene in junctions_by_gene))
